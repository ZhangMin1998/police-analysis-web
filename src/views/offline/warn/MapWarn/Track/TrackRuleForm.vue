...
import BasicInformation from '@/views/common/warn/MapWarn/Rules/Track/BasicInformation'
  ...
  methods: {
    ...
    MapClick (evt) {
      ...
      if (curFeature.name === 'el-mapIconMark') {
        // 点击删除区域
        const { id, y, x } = curFeature.customAttribute
        ...
        setTimeout(() => {
          const layer = this.areaLayers.find(_c => _c.get('id') === id)
          layer && this.$store.state.Ol.oMap.removeLayer(layer)

          const delLayer = this.delLayers.find(_c => _c.get('id') === id)
          delLayer && this.$store.state.Ol.oMap.removeLayer(delLayer)
        }, 0)
    ...
    loadInfo () {
      queryWarnRuleById(this.ID).then(res => {
        ...
        this.$nextTick(() => {
          ...
          this.$refs.BasicInformation.params.gatherNumber = this.curInfo.gatherNumber
          ...
          this.params.fenceRangeList = this.curInfo.fences?.map(_c => {
            return _c.fencePoints.map(ele => {
              const location = gps.GcjWGSConversion({ Lon: ele.x, Lat: ele.y, type: 1 })
              ele.lon = location.longitude
              ele.lat = location.latitude
              return ele
            })
          })
    ...
    handleSubmit () {
      this.$nextTick(() => {
        ...
        this.checkRules(this.params.warningName, () => {
          this.$messageTip.error('请输入预警名称！')
          return false
        })

        if (this.params.gatherNumber < 2) {
          this.$messageTip.error('请确认聚集人数，至少2人！')
          return false
        }
    ...
    selectEnd (feature) {
      // 数组
      const location = feature.getCoordinates()?.[0]
      // 离线地图 经纬度显示
      ...
      const fence = location.map(_c => {
        const curPositionObj = gps.GcjWGSConversion({ Lon: _c[0], Lat: _c[1], type: 2 })
        return { x: curPositionObj?.longitude, y: curPositionObj?.latitude, lon: _c[0], lat: _c[1] }
      })
...
header {
  ...
  // border: 2px solid var(--table-head-bg-color);
  opacity: 0.9;
  position: absolute;
  left: 0;
  top: 0px;
