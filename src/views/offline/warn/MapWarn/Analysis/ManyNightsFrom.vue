  ...
  <div class="rule-wrapper">
    <div class="rule-content">
      ...
      <el-form :rules="rules" ref="ruleForm" class="rule-form" :model="form" label-width="112px">
        ...
        <el-form-item label="深夜出现天数：" prop="nightNum">
...
import { ManyNightsAdd, ManyNightsEdit, ManyNightsQuery } from '@/api/warn'
...
const rules = {
  ...
  nightNum: [
    { required: true, message: '请输入深夜出现天数', trigger: 'blur' },
    { type: 'number', message: '深夜出现天数必须为数字值' }
  ],
...
export default {
  ...
  methods: {
    ...
    confirm () {
      this.$nextTick(() => {
        ...
        if (!this.form.endTime) {
          this.form.endTime = '06:00'
        }

        // 添加 还是 编辑
        const postUrl = this.ID ? ManyNightsEdit : ManyNightsAdd
    ...
    // 获取详情
    getDetail (editId) {
      ManyNightsQuery(editId).then(res => {
        this.$nextTick(() => {
