<template>
<div class='week_warn_wrap'>
  <div class='title'>本周触警</div>
  <div class='chart_wrap' id='weekChart'></div>
</div>
</template>

<script>
import * as echarts from 'echarts'
import { getWeekTotal } from '@/api/warnControl.js'
...
export default {
  ...
  methods: {
    init () {
      let chartDom = document.getElementById('weekChart')
      if (!chartDom) return
      let startMonitorTime = new Date().getTime() - 7 * 24 * 60 * 60 * 1000
      getWeekTotal({
