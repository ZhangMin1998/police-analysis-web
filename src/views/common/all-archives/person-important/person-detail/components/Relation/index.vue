<template>
  <div class="relation">
    <el-tabs tab-position="left" class="tabBtn">
      <!-- 真实身份 -->
      <!-- v-permission="$route.query.label==='重点人档案'? 'important_human:human_relation_real11' : 'normal_human:human_relation_real'" -->
      <!-- 这里使用不了自定义指令，所以只能使用v-if了 -->
      <el-tab-pane v-if="hasPermission($route.query.label==='重点人档案'? 'important_human:human_relation_real' : 'normal_human:human_relation_real')">
        <!-- 左边tab栏 -->
      ...
      <!-- 虚拟账号 -->
      <!-- v-permission="$route.query.label==='重点人档案'? 'important_human:human_virtual_account' : 'normal_human:human_virtual_account'" -->
      <el-tab-pane v-if="hasPermission($route.query.label==='重点人档案'? 'important_human:human_virtual_account' : 'normal_human:human_virtual_account')">
        <div slot="label" style="padding: 0 8px">
          <div class="tabsTitle">
            <div class="circle">
              <div class="inner"></div>
      ...
      <!-- 人物图集 -->
       <!-- v-permission="$route.query.label==='重点人档案'? 'important_human:human_pics' : 'normal_human:human_pics'" -->
      <el-tab-pane v-if="hasPermission($route.query.label==='重点人档案'? 'important_human:human_pics' : 'normal_human:human_pics')">
        <div slot="label" style="padding: 0 8px">
          <div class="tabsTitle">
            <div class="circle">
              <div class="inner"></div>
