        ...
        <ul class="Relation-ul">
          ...
          <li v-for="(item, index) in mapList" :key="index" class="Relation-li">
            <div class="Relation-li-relationShipName">
              <el-input v-model.trim="item.relationShipName" placeholder="请输入描述"></el-input>
            </div>
            <div class="Relation-li-change" @click="handleChangeDirection(index)">
            ...
            <div class="Relation-li">
              ...
              <el-col class="rightArrow" :span="7" :offset="1">
                <img class="rightArrow-icon" src="@/assets/img/icon-rightArrow.png" />
              </el-col>
...
import { getNodeAndRelationList } from '@/api/allArchives'
  ...
  watch: {
    ...
    curNodeName (val) {
      this.mapList = this.mapList.map(_c => {
        if (_c.isRight) {
          // 如果是新增的，那么就去改变startNodeId，如果是编辑的，就不要去改变startNodeId，因为_c里面有startNodeId，改变了会报错
          if(!this.nodeId){
            _c.startNodeId = val
          }
        } else {
    ...
    nodeId: {
      ...
      // 解决点击关系线再切换回来点击节点时不发请求导致没有关系列表的bug
      immediate: true
  ...
  methods: {
    ...
    formatRelationList () {
      const mapList = this.mapList.map(_c => {
        ...
        // startNodeId = ''
        // } else {
        //   startAttrTypeCode = this.nodeList.find(_c => _c.lable == startNodeId)?.typeCode ?? ''
        // }
        // 为1则是编辑时添加关系，为0时则是新增时添加关系
        ...
        if (this.nodeId) {
          ...
          // 新增时添加关系,赋值typeCode,同时清空startNodeId,因为还没有创建的没有startNodeId
          startAttrTypeCode = this.attrTypeCode
          startNodeId = ''
        }
        if (parseFloat(endNodeId).toString() == 'NaN') {
          ...
          // 无论是新增还是编辑都会走这条路,从已有的元素去找到typeCode
