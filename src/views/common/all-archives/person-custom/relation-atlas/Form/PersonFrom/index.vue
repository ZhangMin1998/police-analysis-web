...
import { sexOption, cultureOption, maritalStatusOption } from '@/views/common/all-archives/components/dynamic-form/form-data'
...
export default {
      ...
      handler (val) {
        ...
        if (JSON.stringify(val) !== '{}') {
          ...
          Object.keys(val).forEach(key => {
            if (val[key] && this.PersonFrom.hasOwnProperty.call(key)) {
              obj[key] = val[key]
            }
          })
  ...
  methods: {
        ...
        this.$refs['PersonFrom'].$refs['PersonFrom'].validate(valid => {
          if (valid) {
            ...
            Url(params).then(res => {
              ...
              // bug修复，触发NodeChange事件一定要有RelationId或者canvas，否则接口404
              this.personId
                ? this.$emit('NodeChange', this.RelationId ?? '')
                : this.$emit('NodeChange', res.canvas ?? '')
            }).finally(() => {
              if (this.btnList.length === 1) {
    ...
    go () {
      ...
      this.PersonFrom.sex = sex === '女' ? 'female' : 'male'
