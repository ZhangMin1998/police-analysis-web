    ...
    <div class="main_box">
      <div class="diagram_box" v-if="activeChart === 'diagram'">
        <div class="drawer">
          ...
          >
            <div class="main_box">
              <div class="content_box" v-for="(item,index) in labelList" :key="index" @click="clickLabel(index)">
                  ...
                  <div class="info_box">
                    <div v-for="(e,i) in item" :key="i">
                      <div class="info" v-if="!e.isIcon">
                        <div class="label">
                          <!-- {{e.label + '：'}} -->
                          <EllipsisPop :content="e.label + '：'" :rowNum="1" width="100px"></EllipsisPop>
                        </div>
...
import { getDeductionSingle, getInterPersonal,relationEdgeRank  } from '@/api/analysis'
  ...
  methods: {
    ...
    async obtainGraphData () {
      ...
      if (this.id === 'chart1') { // 人物图谱
        ...
        temp[1].forEach(item => {
          if (item.src === personList[0].vid || item.dst === personList[0].vid) {
            edgeList.push(item)
          }
        })
        personList.forEach(item => {
          edgeList.forEach(e => {
            if (item.vid === e.dst || item.vid === e.src) {
              item.relationship = e.relationship
            }
          })
        })
        // console.log(personList,edgeList)
        // leftChartList.push(personList[0]) // 第一个默认是本人
        // edgeList.forEach(item => {
        //   personList.forEach(e => {
        //     if (item.dst === e.vid || item.src === e.vid && e.vid !== personList[0].vid) {
        //       leftChartList.push(e)
        //     }
        //   })
        // })
      }
      ...
      leftChartList.forEach(item => {
        ...
        if (nameIndex !== -1) {
          ...
          let isSelf = temp.some(item => {
            return item.value === vertex[0].cer_number
          })
          if (!isSelf) { // 如果是本人 就不用在姓名后面添加（关系）
            ...
            // console.log(edge)
            edge.forEach(item => {
              if ((item.dst === vertex[0].vid && item.src === tempVid) || (item.src === vertex[0].vid && item.dst === tempVid)) {
                tempObj.relationship = item.relationship
                tempObj.type = item.type
              }
            })
            // console.log(tempObj,Object.keys(tempObj).length)
            if (Object.keys(tempObj).length) {
              ...
              // console.log(tempObj);
              // console.log(temp)
              // if (tempObj.type !== 'edge_conjugal_relationship') {
              //   temp.forEach(item => {
              //     if (item.label === '姓名') {
              //       item.value = item.value + '（' + relationObj[tempObj.type] + ')'
              //     }
              //   })
              // } else {
              //   temp.forEach(item => {
