...
export default {
  ...
  methods: {
    ...
    formatData (list) {
      ...
      const sArr =
        list[1]?.map(_c => {
          return { source: _c.src + '', target: _c.dst + '', dataRelationShipName: this.dataTypeObj[_c.data_source], personRelationShipName: _c.relationship, ..._c }
        }) ?? []
      ...
      const nArr =
        list[0]?.map(_c => {
          if (_c.type !== 'entity_person') { // 给name赋值  默认在实体上显示
    ...
    initCharts () {
          ...
          formatter: params => {
            ...
            // console.log(params)
            {
              ...
              if (params.data.type === 'entity_person' || params.data.type === 'entity_certificate') {
                ...
                let imgUrl = params.data.avatar_url && params.data.avatar_url.startsWith('image/')
                ...
                <div style="margin-top:0px;color: ${colorObj['--base-text-color-to-999999']};">
                  <div>
                    <div style="display:flex">
      ...
      //       return x.data.relationship ? x.data.relationship + '（' + x.data.dataRelationShipName  + x.data.rank + '次）' : relationObj[x.data.type] + '（' + x.data.dataRelationShipName  + x.data.rank + '次）'
      //     } else if (arr.includes(x.data.type)) {
      //       return relationObj[x.data.type] + '（'+ x.data.dataRelationShipName  + x.data.rank + '次）'
      //     } else {
      //       return this.getVertexName(x.data.dst.split('-')[0]) + '(' + x.data.dataRelationShipName  + x.data.rank + '次）' || ''
      //     }
      //   } else {
      ...
      // 使用刚指定的配置项和数据显示图表。
      this.myChart.setOption(this.option,this.isAsyncLoadData?false:true)
      // if(this.isAsyncLoadData){
      //   setTimeout(() => {
