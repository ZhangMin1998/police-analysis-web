<template>
  ...
  >
    ...
    >
      <BankCardDetail :row="row"></BankCardDetail>
    </pop>

    <CusExport
      method="post"
      fileName="银行业务"
      url="/police/warning/bankCard/export"
...
import { getBankCardData } from '@/api/dataCenter'
import Pop from '@/components/VisualComponents/PopDialog'
import BankCardDetail from './components/bankCardDetail'
...
const allCols = [
  {
    ...
    list: [
      ...
      { label: '银行类型', prop: 'placeType' },
      { label: '办理网点', prop: 'placeName' },
      { label: '网点地址', prop: 'placeAddress' },
      { label: '签名', prop: 'signConfirmFile' },
      { label: '签署日期', prop: 'signDate' },
      { label: '办理时间', prop: 'collectTime' },
      { label: '断卡行动联系人', prop: 'monitorName' },
      { label: '断卡联系人手机', prop: 'monitorPhone' },
...
export default {
  ...
  methods: {
    ...
    exportReq (url, params) {
      params.exportColumns = ['photo', 'name', 'phone', 'cerNumber', 'placeName', 'monitorName', 'monitorPhone', 'collectTime']
      //调用封装方法
      exportUtils
        .exportMethod({
          method: 'post',
    ...
    getTable () {
      ...
      getBankCardData(this.oldParams)
        .then(res => {
