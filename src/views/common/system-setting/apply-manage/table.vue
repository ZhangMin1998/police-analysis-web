  ...
  >
    ...
    >
      <div v-for="(colItem, colIndex) in columns" :key="colIndex">
        ...
        >
          <template slot-scope="scope">
            <span style="margin-right: 8px" v-if="scope.row[colItem.prop] === colItem.onValue">{{
              colItem.activeText || '启用'
            }}</span>
            <span style="margin-right: 8px" v-else>{{ colItem.inactiveText || '禁用' }}</span>
            <el-switch
              class="aside-switch"
              v-model="scope.row[colItem.prop]"
              active-color="#4596E6"
        ...
        >
          <template slot-scope="scope">
            <el-tooltip v-if="scope.row.status === '已失效'" class="item" effect="dark" content="该用户已被删除" placement="top">
              <div>
                {{ scope.row[colItem.prop] }}
              </div>
            </el-tooltip>
            <!-- <el-popover visible-arrow="true" trigger="hover" placement="top" v-if="scope.row.status === '已失效'">
              {{ '该用户已被删除' }}
              <div slot="reference">
                {{ scope.row[colItem.prop] }}
              </div>
            </el-popover> -->
            <font v-else-if="scope.row.status === '已重置'" color="#3AB293">
              {{scope.row[colItem.prop]}}
            </font>
        ...
        >
          <template slot-scope="scope">
            <div v-for="item in colItem.statuList" :key="item.status">
              ...
              <div v-else-if="scope.row[colItem.prop] && scope.row[colItem.prop] != true">
                {{ scope.row[colItem.prop] }}
              </div>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            v-else-if="scope.row.status === '待审批'"
            @click.prevent="btnList[0].btnEvent(scope.row)"
            type="text"
            size="small"
          >
            <i class='iconfont icon-mimazhongzhi' title="重置密码"></i>
          </el-button>
        </template>
      </el-table-column>
...
.icon-xiangqing, .icon-mimazhongzhi {
  font-size: 16px;
