    ...
    <el-row class="opt-row" :gutter="10">
      ...
      <el-col :xs="16" :sm="14" :md="12" :lg="8" :xl="6"
        ><div class="btns-r">
          ...
          <div class="p-button-blue" @click="add" v-permission="'crowd_warning_rules:add_edit_del'">
            新建
          </div>
        </div></el-col
      >
...
const columns = [
  ...
  { label: '聚集人数', prop: 'gatherNumber', minWidth: '5%' },
...
export default {
  ...
  data () {
    return {
      ...
      table: {
        ...
        //     btnEvent: this.edit,
        //     alt: '编辑',
        //     key:'crowd_warning_rules:add_edit_del'
        //   },
        //   {
        //     label: '删除',
        //     type: 'rodeLine',
  ...
  methods: {
    ...
    getList () {
      ...
      queryWarnRulePager({ ...this.table.params, warningType: 'warningGather' })
        .then(res => {
          res.content.forEach(item => {
            item.gatherNumber = item.gatherNumber.toString() // 不转字符串报错
            let tempArr = []
    ...
    // 编辑操作
    edit (row) {
      this.$router.push({ name: 'TrackRuleForm', query: { id: row.id } })
    },
    // 删除操作
    delete (row) {
    ...
    /** 新建按钮 */
    add () {
      this.$router.push({ name: 'TrackRuleForm' })
    }
  }
}
</script>
