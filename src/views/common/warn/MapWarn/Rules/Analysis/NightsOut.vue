<template>
  <div>
    ...
    <pop :pop.sync="smsPop" @beforeClose="closeSms" @beforeOpen="openSms">
      <SmsPanel ref="sms" :smsType="8" @close="closeSms" @submit="submitSms" />
    </pop>
  </div>
</template>
...
import { NightsOutPager, NightsOutDel, addMessageUser } from '@/api/warn'
...
export default {
      ...
      table: {
        ...
        columns: [
          ...
          },
          {
            ...
            type: 'nor',
            renderFun: function (row) {
              if (!row.dayStartTime && !row.dayEndTime) return '--'
              return row.dayStartTime + ' ~ ' + row.dayEndTime
            }
          },
          {
            label: '夜间时间段',
            ...
            type: 'nor',
            renderFun: function (row) {
              if (!row.nightStartTime && !row.nightEndTime) return '--'
              return row.nightStartTime + ' ~ ' + row.nightEndTime
            }
          }
        ],
        btnRender: [
  ...
  methods: {
    ...
    getList () {
      ...
      NightsOutPager(this.table.params)
        .then(res => {
    ...
    // 编辑操作
    edit (row) {
      this.$router.push({ name: 'editNightsOutFrom', query: { id: row.id } })
    },
    // 删除操作
    delete (row) {
    ...
    /** 新建按钮 */
    add () {
      this.$router.push({ name: 'NightsOutFrom' })
    }
  }
}
</script>
