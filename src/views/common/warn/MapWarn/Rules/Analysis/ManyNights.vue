<template>
  <div>
    ...
    <pop :pop.sync="smsPop" @beforeClose="closeSms" @beforeOpen="openSms">
      <SmsPanel ref="sms" :smsType="7" @close="closeSms" @submit="submitSms" />
    </pop>
  </div>
</template>
...
import { ManyNightsPager, ManyNightsDel, addMessageUser } from '@/api/warn'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      ManyNightsPager(this.table.params)
        .then(res => {
    ...
    // 编辑操作
    edit (row) {
      this.$router.push({ name: 'editManyNightsFrom', query: { id: row.id } })
    },
    // 删除操作
    delete (row) {
    ...
    /** 新建按钮 */
    add () {
      this.$router.push({ name: 'ManyNightsFrom' })
    }
  }
}
</script>
