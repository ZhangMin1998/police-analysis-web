<template>
  <div>
    ...
    <pop :pop.sync="smsPop" @beforeClose="closeSms" @beforeOpen="openSms">
      <SmsPanel ref="sms" :smsType="6" @close="closeSms" @submit="submitSms" />
    </pop>
  </div>
</template>
...
import { DeepLivePager, DeepLiveDel, addMessageUser } from '@/api/warn'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      DeepLivePager(this.table.params)
        .then(res => {
    ...
    // 编辑操作
    edit (row) {
      this.$router.push({ name: 'editDeepLiveFrom', query: { id: row.id } })
    },
    // 删除操作
    delete (row) {
    ...
    /** 新建按钮 */
    add () {
      this.$router.push({ name: 'DeepLiveFrom' })
    }
  }
}
</script>
