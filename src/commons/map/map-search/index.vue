<template>
  <div class="search" @click="$emit('clickSearch')">
    <div :class="isChecked || isActive ? 'serach-inner' : 'serach-inner'">
      <div class="search-header">{{ curSearchName }}
...
input::placeholder {
  font-size: 16px;
  color: var(--map-search-placeholder);
}
.icon-sousuo {
  font-size: 24px;
...
.search-list {
  ...
  top: 50px;
  width: 100%;
  background: var(--map-search-result-bg);
}
.search-list-item {
  width: 100%;
  height: 32px;
